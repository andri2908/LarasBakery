        ...
        private bool saveDataTransaction()
        {
            ...
            try
            {
                ...
                for (int i =0;i<detailGridView.Rows.Count;i++)
                {
                    ...
                    if (status == 0 && qty > 0)
                    {
                        ...
                        if (orderQty <= deliveredQty + qty)
                        {
                            ...
                            sqlCommand = "UPDATE SALES_DETAIL SET IS_COMPLETED = 1 WHERE SALES_INVOICE = '" + selectedSalesInvoice + "' AND PRODUCT_QTY = " + orderQty + " AND ID = " + lineItemID;
                            if (!DS.executeNonQueryCommand(sqlCommand, ref internalEX))
                                throw internalEX;
                        }
                    }
                ...
                if (fullfilledItem == detailGridView.Rows.Count)
                {
                    ...
                    sqlCommand = "UPDATE SALES_HEADER SET SALES_ACTIVE = 0 WHERE SALES_INVOICE = '" + selectedSalesInvoice + "'";
                    if (!DS.executeNonQueryCommand(sqlCommand, ref internalEX))
                        throw internalEX;
                }
