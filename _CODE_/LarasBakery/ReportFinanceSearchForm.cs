            ...
            switch (originModuleID)
            {
                case globalConstants.REPORT_FINANCE_IN:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, DJ.JOURNAL_DATETIME AS 'TGL', MA.ACCOUNT_NAME AS 'AKUN', DJ.JOURNAL_NOMINAL AS 'JML', DJ.JOURNAL_DESCRIPTION AS 'DESKRIPSI' " +
                                        ...
                                        "WHERE DJ.ACCOUNT_ID = MA.ACCOUNT_ID AND MA.ACCOUNT_TYPE_ID = 1 AND DJ.BRANCH_ID = " + selectedBranchID + " " +
                ...
                case globalConstants.REPORT_FINANCE_OUT:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, DJ.JOURNAL_DATETIME AS 'TGL', MA.ACCOUNT_NAME AS 'AKUN', DJ.JOURNAL_NOMINAL AS 'JML', DJ.JOURNAL_DESCRIPTION AS 'DESKRIPSI' " +
                                   "FROM DAILY_JOURNAL DJ, MASTER_ACCOUNT MA " +
                                   "WHERE DJ.ACCOUNT_ID = MA.ACCOUNT_ID AND MA.ACCOUNT_TYPE_ID = 2 AND DJ.BRANCH_ID = " + selectedBranchID + " " +
                                   "AND DATE_FORMAT(DJ.JOURNAL_DATETIME, '%Y%m%d') >= '" + dateFrom + "' AND DATE_FORMAT(DJ.JOURNAL_DATETIME, '%Y%m%d') <= '" + dateTo + "'";
                ...
                case globalConstants.REPORT_MARGIN:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, DATE(SH.SALES_DATE) AS 'TGL', SUM((SD.SALES_SUBTOTAL-(SD.PRODUCT_QTY*SD.PRODUCT_PRICE))) AS 'MARGIN' " +
                                                 ...
                                                 "GROUP BY BRANCH_NAME, DATE(SH.SALES_DATE)"; DS.writeXML(sqlCommandx, globalConstants.MarginXML);
