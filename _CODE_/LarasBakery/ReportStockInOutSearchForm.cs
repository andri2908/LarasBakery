...
namespace AlphaSoft
{
    public partial class ReportStockInOutSearchForm : Form
    {
        ...
        private void CariButton_Click(object sender, EventArgs e)
        {
            ...
            {
                case globalConstants.REPORT_PURCHASE_RETURN:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, RH.RP_ID AS 'ID', MS.SUPPLIER_FULL_NAME AS 'SUPPLIER', RH.RP_DATE AS 'TANGGAL', RH.RP_TOTAL AS 'TOTAL', IF(RH.RP_PROCESSED=1,'SUDAH DIPROSES','BELUM DIPROSES') AS 'STATUS', " +
                ...
                case globalConstants.REPORT_SALES_RETURN:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, RH.RS_INVOICE AS 'INVOICE', RH.SALES_INVOICE AS 'NOTAJUAL', MC.CUSTOMER_FULL_NAME AS 'CUSTOMER', RH.RS_DATETIME AS 'TANGGAL', " +
                                           ...
                                           //"GROUP BY RD.ID " + 
                                           "UNION " +
                                           "SELECT '' AS BRANCH_NAME, RH.RS_INVOICE AS 'INVOICE', RH.SALES_INVOICE AS 'NOTAJUAL', 'P-UMUM' AS 'CUSTOMER', RH.RS_DATETIME AS 'TANGGAL', " +
                                           "RH.RS_TOTAL AS 'TOTAL', MP.PRODUCT_NAME AS 'PRODUK', RD.PRODUCT_SALES_PRICE AS 'HARGAJUAL', RD.PRODUCT_SALES_QTY AS 'JMLJUAL', " +
                                           "RD.PRODUCT_RETURN_QTY AS 'JMLRETUR', RD.RS_DESCRIPTION AS 'DESKRIPSI', RD.RS_SUBTOTAL AS 'SUBTOTAL' " +
                    ...
                    break;
                case globalConstants.REPORT_REQUEST_RETURN:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, RH.RP_ID AS 'ID', RH.RP_DATE AS 'TANGGAL', RH.RP_TOTAL AS 'TOTAL', IF(RH.RP_PROCESSED=1,'SUDAH DIPROSES','BELUM DIPROSES') AS 'STATUS', " +
                                           "MP.PRODUCT_NAME AS 'PRODUK', RD.PRODUCT_BASEPRICE AS 'HARGA', RD.PRODUCT_QTY AS 'QTY', RD.RP_DESCRIPTION AS 'DESKRIPSI', RD.RP_SUBTOTAL AS 'SUBTOTAL' " +
                    ...
                    break;
                case globalConstants.REPORT_PRODUCT_MUTATION:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, MH.PM_INVOICE AS 'INVOICE', MH.PM_DATETIME AS 'TANGGAL', MB.BRANCH_NAME AS 'DARI', MB2.BRANCH_NAME AS 'KE', MH.PM_TOTAL AS 'TOTAL', " +
                                   ...
                                   "FROM PRODUCTS_MUTATION_HEADER MH LEFT OUTER JOIN MASTER_BRANCH MB ON (MH.BRANCH_ID_FROM = MB.BRANCH_ID) LEFT OUTER JOIN MASTER_BRANCH MB2 ON (MH.BRANCH_ID_TO = MB2.BRANCH_ID), PRODUCTS_MUTATION_DETAIL MD, MASTER_PRODUCT MP " +
                                   "WHERE MH.PM_INVOICE = MD.PM_INVOICE AND MD.PRODUCT_ID = MP.PRODUCT_ID " + produk +
                    ...
                    break;
                case globalConstants.REPORT_STOCK_DEVIATION:
                    sqlCommandx = "SELECT '' AS BRANCH_NAME, PA.PRODUCT_ADJUSTMENT_DATE AS 'DATE', MP.PRODUCT_ID AS 'ID', MP.PRODUCT_NAME AS 'NAMA PRODUK', MP.PRODUCT_DESCRIPTION AS 'DESKRIPSI', " +
                                    ...
                                    "GROUP BY PA.ID " +
                                    "ORDER BY DEVIASI, PA.PRODUCT_ADJUSTMENT_DATE ASC";
